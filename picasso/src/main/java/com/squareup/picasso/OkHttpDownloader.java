...
public class OkHttpDownloader implements Downloader {
    ...
    @Override
    public Response load(Uri uri, int networkPolicy, PicassoNetworkListener networkListener) throws IOException {
        ...
        if(networkListener != null) {
            //开始下载回调
            networkListener.onNetworkLoadStart(uri.toString(), System.currentTimeMillis());
        }
        com.squareup.okhttp.Response response = client.newCall(builder.build()).execute();
        int responseCode = response.code();
        if (responseCode >= 300) {
            response.body().close();
            ResponseException responseException = new ResponseException(responseCode + " " + response.message(), networkPolicy,
                    responseCode);
            //下载失败回调
            if(networkListener != null) {
                networkListener.onNetworkLoadFail(uri.toString(), System.currentTimeMillis(), responseException, responseCode);
            }
